...
const Property = ({ propertyData: propertyData }) => {
  ...
  const handleLinkClick = () => {
    setPostDetails_(propertyData && propertyData);

    if (currentUser) {
      navigate(`posts/${propertyData && btoa(propertyData.id)}`);
    } else {
      Swal.fire({
        position: 'center',
        icon: 'info',
      ...
      <div className="row" id='post-up'>
        <div className="col-md-8">
          <div className="comment-image-container">
            ...
            <Link to={`students/${propertyData && btoa(propertyData.user.id)}`} id='username-link'>
              <span className="comment-author date-name-group">{propertyData && propertyData.user.name}</span>
            </Link>
            <span className="created date-name-group">{propertyData && propertyData.complainDate ? propertyData.complainDate.substring(0, 10) : null}</span>
          </div>
        </div>
        ...
        <div onClick={handleLinkClick}>
          ...
          >
            ...
            <div className="comment-text">
              {propertyData && propertyData.description ? propertyData.description.substring(0, 120) : null}
              {propertyData && propertyData.description.length > 200 ? <span id='readMore'> Read more...</span> : null}
            </div>
            <hr />
          </Link>
        </div>
